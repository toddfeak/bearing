# Read Path Migration Strategy

How to transition from the current trait-based read path to the target architecture in [read_path_architecture.md](read_path_architecture.md) without breaking the codebase mid-migration. This document is about strategy and sequencing, not the destination shape (covered by the architecture doc) or the immediate VInt optimization trigger (covered by [cursor_read_path.md](cursor_read_path.md)).

## Approach: Parallel Build, Module Isolation, Per-Codec Migration

Build the new read-path types in a separate module alongside the old ones. Migrate codec readers one at a time. Delete the old types in a final cleanup commit. Every commit in between compiles and passes tests.

The change is large enough that a single big-bang rewrite is risky and unreviewable. Parallel build trades temporary code duplication for the ability to pause, verify, and roll back at any point.

## Module Isolation

Place the new types under `src/store2/`. The new `IndexInput<'a>` struct and the existing `IndexInput` trait collide on the name; module qualification keeps them disambiguated during the overlap period.

If module qualification gets confusing in practice, give the new struct a temporary distinct name (`FileView<'a>`, `ReadCursor<'a>`, etc.) and rename it to `IndexInput` in the final cleanup commit, after the trait is deleted.

## Visibility During Migration

Mark all types and methods in `src/store2/` as `pub` from the start. While building out the parallel module, new types are added before they are referenced from outside the module — without `pub`, they would generate continuous `dead_code` warnings (or require per-item `#[expect(dead_code)]` annotations on every new item).

Treat visibility as an audit step in the cleanup commit, not during build-up. Once the migration completes and all callers are wired through, tighten each type and method to the narrowest appropriate visibility (`pub`, `pub(crate)`, `pub(super)`, or private) based on actual external usage.

## What Builds in Parallel Without Touching Old Code

These are all new types in the new module — no conflict with the existing read path:

- `FileBacking` enum (`Mmap` or `Owned`)
- The new `IndexInput<'a>` struct with all inherent read methods
- Free function for checksum verification
- Unit tests for all of the above

No generic segment-wide container is introduced; each codec reader will hold its own typed `FileBacking` fields at migration time (see architecture doc, Layer 3).

This entire layer can ship and pass tests before any codec reader knows it exists.

## What Requires Touching Shared Code

Two shared touchpoints, both additive:

- **`Directory` trait gains a new method** (e.g., `open_file -> FileBacking`) alongside the existing `open_input -> Box<dyn IndexInput>`. Production directories implement both. Old method stays until the final cleanup commit.
- **`SegmentReader` constructs codec readers either way.** During migration, some codec readers are constructed via the old path, some via the new. Both paths coexist in `SegmentReader::open_from_directory`.

`CompoundDirectory` is the awkward case: it currently slices into a parent `Box<dyn IndexInput>`. It needs to support the new `open_file` method by slicing into the parent's bytes. Either implement it in parallel (both methods working independently) or gate compound migration until the underlying directory layer migrates first.

## Per-Codec Migration

Each codec reader migrates in its own commit. The pattern:

1. `SegmentReader` constructs that codec reader the new way (calls `open_file`, builds new `IndexInput`)
2. The codec reader's struct fields and method signatures switch to the new types
3. The codec reader's tests update to construct the new input type
4. e2e still passes because every other codec reader is unchanged

Suggested order (simplest first to validate the pattern, complex last):

1. ~~`points_reader`~~ — migrated (commit b9ed958)
2. ~~`stored_fields_reader`~~ — migrated (this commit); `term_vectors_reader` partially migrated (its `.tvm` meta path only, since it shares `FieldsIndexReader` with stored_fields); `DirectReader` / `DirectMonotonicReader` in `packed_readers.rs` also moved to `store2::IndexInput`
3. ~~`norms_reader`~~ — migrated (this commit); `IndexedDISI` migrated alongside since norms is its only runtime caller; search-side types (`Weight`, `ScorerSupplier`, `Scorer`, `BulkScorer` impls) gained a `'r` lifetime to carry the now-borrowed norms iterator from `LeafReaderContext`
4. ~~`doc_values_producer` and `term_vectors_reader` (full migration of the `.tvd`/`.tvx` paths)~~ — `term_vectors_reader` `.tvd`/`.tvx` migrated (this commit); `doc_values_producer` is a field-type-only swap — `data` field moved to `FileBacking`, `.dvm`/`.dvd` paths and helper signatures moved to `store2::IndexInput`, but the five `DocValuesProducer::get_*` methods remain `todo!()`. Full disk-backed doc values read-path implementation (iterator types, DocValuesEntry offset/encoding metadata, IndexedDISI integration) is deferred to a future step.
5. ~~`postings_reader`~~ — migrated (this commit); `BlockPostingsEnum` gained an `'a` lifetime borrowed from `PostingsReader.doc`. `src/encoding/pfor.rs` is unchanged — `pfor_decode` / `for_delta_decode` still take `&mut dyn Read`, preserving the encoding package's "built against a standard Rust trait" property. Instead, `store2::IndexInput` gained adapter methods (`read_group_vints`, `pfor_decode`, `for_delta_decode`) that delegate by passing `&mut self.cursor` to the Read-based encoding functions. Search-side types already carried the `'a` from the norms migration, so only field types needed updates.
6. ~~`blocktree_reader` and `segment_terms_enum`~~ — migrated (this commit); `FieldReader` split into `FieldMeta` (lightweight metadata, owned by `BlockTreeTermsReader`) and `FieldReader<'a>` (borrowed view constructed on demand). `SegmentTermsEnum<'a>` and `SegmentTermsEnumFrame` now carry `store2::IndexInput<'a>` for `.tim` + `.tip` slices. `TrieReader<'a>` replaces its `Box<dyn RandomAccessInput>` with an `IndexInput<'a>` over the field's `.tip` region; `RandomAccessInput` has no remaining live callers. `SegmentReader::terms(field)` now returns `Option<FieldReader<'_>>` instead of `Option<&dyn Terms>`, so the four call sites (search/term_states, search/index_searcher, bin/listterms, bin/generate_summary) explicitly import the `Terms` trait to call its methods. `.tmd` is validated with `verify_checksum`; `.tim` and `.tip` use `retrieve_checksum` for O(1) footer sanity, matching the postings pattern. `store2::IndexInput` gained a `cursor_mut()` accessor so codec-level `read_compressed` can pass `&mut Cursor<&[u8]>` to `lz4::decompress_from_reader` / `lowercase_ascii::decompress_from_reader` without wrapping each compressor as an `IndexInput` adapter method.
7. ~~`segment_info_format`, `field_infos_format`, `segment_infos`~~ — migrated; three one-shot metadata readers (`.si`, `.fnm`, `segments_N`). Each now calls `directory.open_file` to obtain a `FileBacking`, runs `store2::codec_footers::verify_checksum` for full-file integrity up front, then parses the body through a `store2::IndexInput` over the pre-footer prefix (`bytes.len() - FOOTER_LENGTH`). `store2::codec_headers::check_index_header` replaces `codec_util::check_index_header` for `.si` and `.fnm`; `check_header` is used for `segments_N` since the segment infos ID is discovered mid-file. No retained handles, no struct-shape changes — purely a function-body swap plus imports. `ChecksumIndexInput` and `codec_util::check_footer(&mut ChecksumIndexInput)` are now used only by `compound_reader` and internal `codec_util` tests.

8. `compound_reader` (Directory implementation) — now the only remaining old-path codec caller. Gated on an open design question about `FileBacking` for sub-range mmap views: compound sub-file offsets are not page-aligned, so "each sub-file is its own `FileBacking::Mmap(Mmap)`" does not work directly. Options on the table are (a) extend `FileBacking::Mmap` to carry an intra-mapping offset + length, (b) map `.cfs` once and share via `Arc<Mmap>` + range, or (c) accept `FileBacking::Owned(Vec<u8>)` per sub-file (copy). Decision deferred; conversation paused after Step 3.

## Final Cleanup Commit

Once every codec reader is migrated:

- Delete old `DataInput`, `IndexInput`, `RandomAccessInput` traits
- Delete `MmapIndexInput`, `ByteSliceIndexInput`, `FSIndexInput`, `SliceReader`, `ChecksumIndexInput`
- Delete the old `Directory::open_input` method
- Delete the old `ReadEncoding` blanket trait if no longer used
- Move `verify_checksum` from `src/store2/checksum.rs` into `src/codecs/codec_util.rs` alongside `write_footer`; drop the local `FOOTER_MAGIC` / `FOOTER_LENGTH` copies in favor of the existing `codec_util` constants. The footer format is codec-specific wire format, not generic byte I/O, and belongs next to its writer.
- Move new module contents to `src/store/`, rename types if temporary names were used
- Update imports across the codebase

## Cost

Temporary code duplication during the migration window. Two implementations of "byte slice with position tracking" coexist. The codebase is larger and slightly more confusing while the migration is in flight.

## Where This Plan Could Fail

The plan assumes shared touchpoints are additive. If the new types require a change to `Directory` or `SegmentReader` that breaks old codec readers, parallel-build falls apart and we have to choose between halting or going big-bang. From inspection of the existing code, the only shared touchpoints look additive (new method on Directory, new branches in SegmentReader). The first codec migration is the cheapest way to validate this assumption — if it forces a breaking change to `Directory`, the strategy needs revisiting before continuing.

## Success Criteria

The migration delivers value if it produces measurable improvement against current baselines:

- **Query performance:** baseline ~57 us/query (4.2x vs Java) measured by `./tests/compare_query_perf.sh -docs /tmp/gutenberg-large-500`. Target is meaningful improvement.
- **Term iteration:** baseline ~1.0s for 33M terms measured via `listterms` (iteration-only mode, no `BTreeSet`/output) against a large index. Target is meaningful improvement.

The VInt optimization unlocked by `Cursor`/`BufRead` is the primary expected driver of improvement, with secondary contributions from removed dynamic dispatch and removed Arc churn on slice operations.

## Follow-ups

- **Keep `/encoding` pure (Read/Cursor) without bloating `IndexInput` with a method per decode.** The postings migration added three adapter methods (`read_group_vints`, `pfor_decode`, `for_delta_decode`) on `store2::IndexInput` that delegate to `&mut dyn Read`-taking functions in `/encoding`. This preserves `/encoding`'s dependency on standard Rust traits, but grows `IndexInput`'s surface with one method per encoding format. As more codecs migrate (blocktree, compound) and more encodings become borrowed-byte callers, this pattern will get heavier. Options to explore once migration is complete: (a) `impl Read for IndexInput` + drop the adapters (lets the `ReadEncoding` blanket trait light up all methods automatically, but re-introduces the `Read` supertrait concern the architecture doc called out); (b) expose `IndexInput::cursor_mut()` and let callers pass `&mut Cursor<&[u8]>` to encoding functions directly (concrete type, no vtable, but shifts responsibility to every call site); (c) switch encoding functions to take `&mut Cursor<&[u8]>` instead of `&mut dyn Read` (still standard Rust, but narrows the abstraction). None of these should be decided mid-migration — pick one after the full picture is visible.

- **`IndexInput::unnamed` constructor for sites without a meaningful name.** `store2::IndexInput<'a>` carries a `name: String`, but the only production reader is the `Debug` impl — nothing uses it in error messages or logs. Many test sites and a few production sites (temporary views inside `seek_exact_in_block`, `scan_to_floor_block`) pass a contrived name just to satisfy the constructor. Add an `unnamed(bytes: &'a [u8])` constructor that defaults the name to a sentinel like `"UNSET"`, and use it wherever the name doesn't carry information. Keep the named `new(name, bytes)` constructor for sites that do name their inputs (`terms_in`, `index input`, `.tim` / `.tip` / `.tmd` at open time). Low-risk, purely additive. Revisit if `name` ever gets wired into error messages — at that point either commit to names everywhere or drop the field entirely.

- **Cleaner API for deriving a fresh `IndexInput` without exposing the underlying `&[u8]`.** Every `IndexInput<'a>` has two pieces of state: a name and a cursor position. Several read-path call sites want a *derived* view that shares the underlying bytes but has its own fresh cursor at position 0 and a different name — either over the full length (e.g. `seek_exact_in_block`, `scan_to_floor_block` constructing a temporary reader for one block) or over a sub-range (e.g. the blocktree `FieldReader` handing out `.tim` / `.tip` views per query). Today the blocktree migration works around this by storing raw `&'a [u8]` fields on `FieldReader<'a>` and calling `IndexInput::new(name, bytes)` at each use site; other sites call `sub_input(name, 0, length)`. Both leak the byte-slice primitive into codec code and force callers to know that a "copy" means "new cursor + new name over the same bytes". Options to explore: (a) add `IndexInput::view(name)` (same bytes, fresh cursor, new name) and `IndexInput::view_slice(name, offset, length)` (sub-range, fresh cursor, new name) and stop exposing `&[u8]` from codec readers; (b) make derived views the only way to obtain an `IndexInput` borrowed from an existing one, so codec structs can hold an `IndexInput<'a>` as the "source" without leaking bytes and without accidentally sharing cursor state. Pick one after the full codec migration is visible so the surface is driven by real call sites, not speculation.
