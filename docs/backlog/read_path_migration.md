...
- ~~**`IndexInput::unnamed` constructor for sites without a meaningful name.**~~ Landed: `IndexInput::unnamed(bytes)` defaults the name to a `"<unnamed>"` sentinel. Adopted at all unit-test placeholder sites and at production sites that previously passed file-extension labels (`"nvd"`, `"fdt"`, `"doc"`, `"direct"`). Sites with semantically meaningful names (`"floor"`, `"stats"`, `"meta"`, `"trie index"`, `"suffix_lengths"`, etc.) keep `IndexInput::new`.

- ~~**Cleaner API for deriving a fresh `IndexInput` without exposing the underlying `&[u8]`.**~~ Landed (partial): `sub_input` renamed to `view` (same `(name, offset, length)` signature). A default-named variant was considered and dropped — every production sub-view today carries a semantic name worth keeping (`"disi-blocks"`, `"floor"`, `"stats"`, `"meta"`, etc.). Revisit if a real call site without a meaningful name appears.
