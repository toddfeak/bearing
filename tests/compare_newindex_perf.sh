...
# Usage: compare_newindex_perf.sh [-docs DIR] [--debug] [--threads N] [--1t] [--no-verify] [--compound]
#
# Options:
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        *)
            ...
            echo "Usage: compare_newindex_perf.sh [-docs DIR] [--debug] [--threads N] [--1t] [--no-verify] [--compound]"
            exit 1
            ;;
    esac
done
...
echo "========================================"
echo "  Java vs Rust Newindex Comparison"
echo "========================================"
echo ""
...
echo "Building Rust newindex_demo ($BUILD_MODE)..."
cargo build --bin newindex_demo $CARGO_FLAGS --manifest-path "$PROJECT_DIR/Cargo.toml" 2>&1 | tail -1
NEWINDEX_DEMO="$PROJECT_DIR/target/$BUILD_MODE/newindex_demo"
echo ""
...
if [[ -n "$RUN_1T" ]]; then
    ...
    run_index "RUST newindex_demo ($BUILD_MODE, 1 thread)" \
        "$NEWINDEX_DEMO -docs $DOCS_DIR -index $RUST_1T_INDEX --threads 1 $RUST_COMPOUND_FLAG" \
        "$RUST_1T_INDEX"
...
run_index "RUST newindex_demo ($BUILD_MODE, $MT_THREADS threads)" \
    "$NEWINDEX_DEMO -docs $DOCS_DIR -index $RUST_MT_INDEX --threads $MT_THREADS $RUST_COMPOUND_FLAG" \
    "$RUST_MT_INDEX"
